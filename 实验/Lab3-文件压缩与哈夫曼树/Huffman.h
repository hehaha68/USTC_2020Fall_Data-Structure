...
typedef struct Node {//Huffman结构
    long    w;//w为权值
    short   p, l, r, add; //p为parent，l为左孩子，r为右孩子，add存放结点存储位置
...
/*********函数声明*********/
int compare(const Node& a, const Node& b);//比较权值
long Frequency(FILE* in, long frequency[]);//统计字符出现频率
int Select(HTnp ht, int n, int* s1, int* s2);//选取结点权值最小的结点
int Create_HFTree(long w[], int n, HTNode ht[]);//创建Huffman Tree
int Encode_HFTree(HTnp htp, int n, HFCode hc[]);//创建Huffman Code
void Mini_HFTree(FILE* in, short mini_ht[][2]);//创建小型Huffman Tree(用于解压)
/*************************/
...
//int Select(HTnp ht, int n, int* s1, int* s2) { //选取结点权值最小的树//改用优先队列实现此功能
//    int i, x;
...
int Create_HFTree(long w[], int n, HTNode ht[]) { //创建哈夫曼树//利用优先队列头文件参杂c++
    int m, i;
    int s1, s2;
    HTNode S1, S2;
    ...
    for (i = n; i < m; i++) {
        ...
        Priority_Queue<Node>::dequeue(Q, S2);//依次选取两个权值最小的点
        s1 = S1.add;
        s2 = S2.add;
        ht[s1].p = ht[s2].p = i;
        ht[i].l = s1;
        ...
        ht[i].w = ht[s1].w + ht[s2].w;
        ht[i].add = i;
        Priority_Queue<Node>::enqueue(Q, ht[i]);//加入一个合成结点
    }
